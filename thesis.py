...
def parse_micom_undirected(file, strength, output):
    ...
    handle2 = open("%s/%s.str%s.undirected.txt"%\
                   (output, sample, str(strength).replace(".", "")), "w")
...
def MDSM(input_folder, strength, output, prefix):
    ...
    handle = open("%s/%s.%ilayer.str%s.txt"\
                  %(output, prefix, len(filelist), str(strength).replace(".", "")), "w")
...
def fmt_process(recovery, output, prefix, strength):
    ...
    # get stable state abundance
    if ("nan" in list(map(str, x_fmt))) or ("inf" in list(map(str, x_fmt))):
        for item in xx_fmt[::-1]:
            # At stoppinng point, species abundance in 0-200g
            neg_count = len(list(filter(lambda x: (x < 0), item)))
...
def main():
    ...
    #
    interaction_command = subparsers.add_parser("interaction", \
                                                help="Bacterial interaction inference using MICOM")
    interaction_command.add_argument("input", metavar="input_file", type=str, \
                                     help="Input file of a list of taxonomic classification files")
    interaction_command.add_argument("-m", "--medium", default="medium.csv", \
                                     help="Medium CSV file, default medium.csv")
    interaction_command.add_argument("-d", "--model", default="dbs", \
                                     help="Metabolic model database, default dbs")
    interaction_command.add_argument("-p", "--percentage", default=0.1, type=float, \
                                     help="Percentage of species removed, default 0.1")
    interaction_command.add_argument("-f", "--flag", default=True, \
                                     help="Calculate growth rate, default True")
    interaction_command.add_argument("-o", "--output", default="output_interaction", \
                                     help="output folder, default output_interaction")
    #
    driver_command = subparsers.add_parser("drivers", \
                                           help="Find driver nodes using MDSM")
    driver_command.add_argument("input", metavar="input_folder", type=str, \
                                help="Input Folder of Bacteria Interaction Networks")
    driver_command.add_argument("-s", "--strength", default=0.2, type=float, \
                                help="Threshold of Interaction Strength, default 0.2")
    driver_command.add_argument("-p", "--prefix", default="driver_nodes", \
                                help="Output file prefix, default driver_nodes")
    driver_command.add_argument("-o", "--output", default="output_driver", \
                                help="Output file folder, default output_driver")
    #
    fmt_command = subparsers.add_parser("fmt_all", \
                                        help="Simulate FMT process following GLV models")
    fmt_command.add_argument("input", metavar="input_file", \
                             help="Input Disease and Donor Samples File Address")
    fmt_command.add_argument("-m", "--medium", default="medium.csv", \
                             help="Medium CSV file, default medium.csv")
    fmt_command.add_argument("-d", "--model", default="dbs", \
                             help="Metabolic model database, default dbs")
    fmt_command.add_argument("-p", "--percentage", default=0.1, type=float, \
                             help="Percentage of species removed, default 0.1")
    fmt_command.add_argument("-s", "--strength", default=0.2, type=float, \
                             help="Threshold of Interaction Strength, default 0.2")
    fmt_command.add_argument("-o", "--output", default="fmt_output", \
                             help="Output file folder, default fmt_output")
    #
    #
    fmt_driver_command = subparsers.add_parser("fmt_driver", \
                                               help="Simulate driver species transplantation process following GLV models")
    fmt_driver_command.add_argument("input", metavar="input_file", \
                                    help="Input Disease Samples File Address")
    fmt_driver_command.add_argument("-i", "--driver", \
                                    help="Input Driver Species")
    fmt_driver_command.add_argument("-a", "--amount", default=4, type=float, \
                                    help="Input amount of driver species, default 4g")
    fmt_driver_command.add_argument("-m", "--medium", default="medium.csv", \
                                    help="Medium CSV file, default medium.csv")
    fmt_driver_command.add_argument("-d", "--model", default="dbs", \
                                    help="Metabolic model database, default dbs")
    fmt_driver_command.add_argument("-p", "--percentage", default=0.1, type=float, \
                                    help="Percentage of species removed, default 0.1")
    fmt_driver_command.add_argument("-s", "--strength", default=0.2, type=float, \
                                    help="Threshold of Interaction Strength, default 0.2")
    fmt_driver_command.add_argument("-o", "--output", default="fmt_driver_output", \
                                    help="Output file folder, default fmt_driver_output")

    #
    fmt_only_command = subparsers.add_parser("fmt_only", \
                                             help="Simulate FMT process following GLV models")
    fmt_only_command.add_argument("input_file", metavar="input-file", \
                                  help="Input file prefix")
    fmt_only_command.add_argument("-s", "--strength", default=0.2, type=float, \
                                  help="Threshold of Interaction Strength, default 0.2")
    fmt_only_command.add_argument("-p", "--prefix", default="fmt_only_output", \
                                  help="Output file prefix")
    fmt_only_command.add_argument("-o", "--output", default="fmt_output", \
                                  help="Output file prefix")
    #
    ...
    #
    elif args.subparser_name == "drivers":
        if os.path.exists(args.output) == False:
            os.mkdir(args.output)
        MDSM(args.input, float(args.strength), args.output, args.prefix)
    #
    elif args.subparser_name == "fmt_all":
        if os.path.exists(args.output) == False:
            os.mkdir(args.output)
        output = args.output
